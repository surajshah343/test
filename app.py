...
# Removed hidden non-breaking space after 252
...
def load_data(ticker, start_date):
    try:
        ...
        # Calculate Log Returns correctly for the whole dataframe
...
def engineer_features(df):
    df = df.copy()
    # Lagged returns and technical ratios
    ...
    # Logic fixed to calculate residual alpha relative to moving average
    df['Drift_50'] = df['Log_Ret'].rolling(50).mean()
    df['Target_Residual'] = df['Log_Ret'].shift(-1) - df['Drift_50']
    
    # Periodic time features - Fixed datetime access
...
def train_model(X, y):
    ...
    tuner = RandomizedSearchCV(
        ...
        param_distributions={'max_depth': [3, 5, 10], 'n_estimators': [50, 100, 200]},
...
if not os.path.exists(MODEL_FILE):
    ...
    with open(META_FILE, 'w') as f:
        # Fixed meta date extraction
        json.dump({"last_trained": ml_data['Date'].max().strftime("%Y-%m-%d")}, f)
else:
...
def run_simulation(_model, _historical_df, _n_days, _n_sims):
    ...
    # Use log returns for drift calculation
    ...
    # We need these to update features in the loop
    ...
    for d in range(_n_days):
        ...
        # Stochastic Update: drift + alpha + noise 
        ...
        # Update current_log_ret for next iteration (simplified as mean of sims)
...
fig = go.Figure()
# Fixed x-axis for historical data
fig.add_trace(go.Scatter(x=ml_data['Date'].tail(252), y=ml_data['Close'].tail(252), name='Historical', line=dict(color='black')))
